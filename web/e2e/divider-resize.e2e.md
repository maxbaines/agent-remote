...
Resize in just-terminal uses two clocks:
...
```bash
playwright-cli --raw eval "JSON.stringify({cols: window.__justTerminal.snapshot(2).cols, rows: window.__justTerminal.snapshot(2).rows})" > /tmp/before.json
...
```bash
playwright-cli --raw eval "JSON.stringify({cols: window.__justTerminal.snapshot(2).cols, rows: window.__justTerminal.snapshot(2).rows})" > /tmp/after.json
