...
This is the clean development cutover for Agent Remote desktop v1. It combines browser-local
Keybindings, explicit tab and directional Split Commands, macOS-style `Cmd+K`, and the fixed
Agent Remote appearance. It runs the Gateway, Session Owner, and Terminal Sessions as the
...
`AGENT_REMOTE_SKIP_SAFARI=1 make verify-desktop-v1`. On a non-macOS builder the variable simply
...
test ! -x bin/agent-remote || cp bin/agent-remote tmp/desktop-v1-cutover/agent-remote.previous
make build
install -m 0755 bin/agent-remote tmp/desktop-v1-cutover/agent-remote.current
```
...
export AGENT_REMOTE_DESKTOP_V1_RUNTIME="$HOME/.agent-remote-desktop-v1-cutover"
mkdir -p "$AGENT_REMOTE_DESKTOP_V1_RUNTIME"
XDG_RUNTIME_DIR="$AGENT_REMOTE_DESKTOP_V1_RUNTIME" \
  tmp/desktop-v1-cutover/agent-remote.current serve \
...
XDG_RUNTIME_DIR="$AGENT_REMOTE_DESKTOP_V1_RUNTIME" \
  tmp/desktop-v1-cutover/agent-remote.current doctor
ps -axo pid,uid,user,command | grep '[a]gent-remote.*\(serve\|sessiond\)'
stat -f '%Su %u %N' "$AGENT_REMOTE_DESKTOP_V1_RUNTIME/agent-remote/sessiond.sock"
```
...
2. Confirm `tmp/desktop-v1-cutover/agent-remote.previous` exists and is executable.
3. Start that binary as the same non-root user with a newly created runtime directory and a free
   loopback port.
4. Confirm `/api/health`, `agent-remote doctor`, browser startup, and a new Pane running `id -u`.
5. Stop the rollback processes. Restart `agent-remote.current` with another fresh runtime to return
   to desktop v1.
