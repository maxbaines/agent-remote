...
STABLE_BIN := $(HOME)/.local/bin/agent-remote
...
build: web
	go build -o bin/agent-remote ./cmd/agent-remote

# Dev mode: demo backend + demo frontend + Vite watch (agent-remote UI) + Caddy + air (Go hot-reload).
...
#   - Vite rebuilds web/dist on agent-remote frontend changes
#   - air detects web/dist + Go changes and rebuilds/restarts agent-remote DEV on
...
#     so agent-remote sees a loopback peer (auth-bypass) and the host Caddy can reach it
#   - Production (systemd) runs separately on :9090 from ~/.local/bin/agent-remote — undisturbed.
# First run: `make demo-install` to install demo npm deps.
# Exposed by the HOST Caddy at https://agent-remote-dev.ampbox.io (see /mnt/services/agent-remote-dev.caddy)
...
dev:
	...
	echo "dev stack:"; \
	echo "  agent-remote       http://127.0.0.1:9091  (air hot-reload)"; \
...
# Dev-local mode: fully isolated second agent-remote instance on THIS Mac only.
#   - own binary   bin/agent-remote-dev (air-managed, rebuilds on Go/web changes)
#   - own port     127.0.0.1:8313  (distinct from prod 8311 and remote-VM dev 8312)
#   - own runtime  ${TMPDIR:-/tmp}/agent-remote-dev-local/ (XDG_RUNTIME_DIR override) --
#     sessiond socket/log/server.url all live here instead of the default
#     $TMPDIR/agent-remote-<uid>/ where production's sessiond lives, so production is
...
#     (e.g. tmp/agent-remote-dev-runtime) because a worktree-local path can push the
...
# Ctrl-C stops the Vite watcher and air (which tears down its bin/agent-remote-dev
...
# therefore its supervised bin/agent-remote-dev serve process) could survive that,
...
# by deleting $${TMPDIR:-/tmp}/agent-remote-dev-local/ if ever desired.
# Requires: air (falls back to $(HOME)/go/bin/air if not on PATH).
dev-local:
	@mkdir -p tmp
	@export XDG_RUNTIME_DIR="$${TMPDIR:-/tmp}"; \
	XDG_RUNTIME_DIR="$${XDG_RUNTIME_DIR%/}/agent-remote-dev-local"; \
	...
	echo "  agent-remote-dev   http://127.0.0.1:8313  (air hot-reload)"; \
...
# Start demo services only — assumes agent-remote is already running at :9091.
...
# This is what systemd runs — separate from ./bin/agent-remote used by `make dev`.
# Usage: git pull && make install-stable
#        systemctl --user restart agent-remote agent-remote-sessiond
install-stable: web
	...
	go build -o $(STABLE_BIN) ./cmd/agent-remote
	@echo "Installed: $(STABLE_BIN)"
	@echo "Restart services: systemctl --user restart agent-remote agent-remote-sessiond"
