...
// level just-terminal has, so there is nothing else to differentiate on.
const (
	ClientWeb = "just-terminal-web"
	...
	// validateRedirectURI can recognize "this is the just-terminal-mcp client"
...
// NewClientStore returns the fixed ClientStore containing just-terminal-web and
// just-terminal-mcp. There is no dynamic client registration (see design doc
// "Alternatives Considered"). webRedirectURI is the exact-match redirect
// URI for the web client, supplied by cmd/just-terminal's webRedirectURIFor: the
...
// design doc's "Client model": just-terminal-web requires an exact string match
// (handled by the clientDomain == redirectURI case, which covers any
// client whose Domain isn't the just-terminal-mcp sentinel). just-terminal-mcp allows
// any port on http://127.0.0.1 — scheme and host are still exact.
//
// Path is intentionally NOT validated for just-terminal-mcp here: the exact
...
func validateRedirectURI(clientDomain, redirectURI string) error {
	...
	if clientDomain == mcpDomainSentinel {
		...
		if u.Scheme != "http" {
			return fmt.Errorf("authserver: redirect_uri scheme must be http for just-terminal-mcp")
		}
		if u.Hostname() != "127.0.0.1" {
			return fmt.Errorf("authserver: redirect_uri host must be 127.0.0.1 for just-terminal-mcp")
		}
