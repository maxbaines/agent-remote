...
// getConfig returns the full resolved just-terminal configuration as a JSON string.
...
func registerConfigTools(srv *Server) {
	...
	srv.Register(
		"get_config",
		"get the full resolved just-terminal configuration (theme, font, terminal, keys, workspace, driver)",
		map[string]any{
			"type":       "object",
	...
	srv.Register(
		"update_config",
		"update just-terminal configuration; accepts a partial config object, merges it, writes to disk, and broadcasts to all connected clients; example: {\"changes\": {\"theme\": {\"palette\": \"dracula\"}, \"font\": {\"size\": 15}}}",
		map[string]any{
			"type": "object",
			"properties": map[string]any{
